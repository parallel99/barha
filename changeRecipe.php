...
<div class="form-container container recipe-container recipe-height">
    <form method="post" enctype="multipart/form-data">
        <div class="form-group">
            ...
            <input type="text" class="form-control" name="name" autocomplete="off" id="name" placeholder="<?= _NAME ?>" value="<?php echo $recipe->name; ?>" required>
        </div>
        <div class="form-group">
        ...
        <div class="ingredients-group">
            ...
            foreach ($ingredients as $key => $value) {
                ...
                <div class="form-group">
                    ...
                    <input type="text" class="form-control ui-autocomplete-input upload-ingredients-name" name="ingredients<?php echo $i; ?>" id="ingredients<?php echo $i; ?>" placeholder="<?= _INGREDIENT ?>" value="<?php echo $value->name; ?>" autocomplete="off">
                    <input type="number" class="form-control ui-autocomplete-input upload-ingredients-db" name="db<?php echo $i; ?>" id="db<?php echo $i; ?>" placeholder="<?= _QUANTITY ?>" min="1" max="5000" value="<?php echo $value->quantity; ?>" autocomplete="off">
            ...
            ?>
            <div class="form-group">
                <input type="text" class="form-control ui-autocomplete-input upload-ingredients-name" name="ingredients<?php echo $i; ?>" id="ingredients<?php echo $i; ?>" placeholder="<?= _INGREDIENT ?>" autocomplete="off">
                <input type="number" class="form-control ui-autocomplete-input upload-ingredients-db" name="db<?php echo $i; ?>" id="db<?php echo $i; ?>" placeholder="<?= _QUANTITY ?>" min="1" max="5000" autocomplete="off">
                <select class="form-control ui-autocomplete-input upload-ingredients-unit" id="unit<?php echo $i; ?>" name="unit<?php echo $i; ?>" autocomplete="off" data-live-search="true">
                    <?php
                    foreach (_UNITS as $unit) {
                        echo "<option value='" . $unit . "'>" . $unit . "</option>";
        ...
        <script>
            $('.ingredients-group').on('input', function () {
                ...
                if ($("div.ingredients-group div:last-child > input").val() !== "" && length < 25) {
                    let inputs = "<div class='form-group'><input type='text' class='form-control upload-ingredients-name' name='ingredients" + (length + 1) + "' id='ingredients" + (length + 1) + "' placeholder='<?= _INGREDIENT ?>'> ";
        ...
        <div class="form-group">
            ...
            <textarea class="form-control making-textarea" name="making" placeholder="<?= _DIRECTIONS ?>" rows="10" required><?php echo $recipe->making; ?></textarea>
        </div>
